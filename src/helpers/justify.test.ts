import { processJustification } from "./justify";

test("should justify text correctly", () => {
  ...
  // Note: The sample output from gdrive was modified there
  // because we don't test for the exact justification algo;
  // the key is still the number of chars per line
...
suivent encore dans le silence de la nuit, à la douceur prochaine du retour.`;
  ...
  expect(processJustification(input)).toBe(expectedOutput);
});
...
test("should handle empty input", () => {
  ...
  expect(processJustification(input)).toBe(``);
});

test("should throw error for invalid input", () => {
  expect(() => processJustification(null as any)).toThrow(
    "Invalid input: Text must be a string."
  );
  expect(() => processJustification(undefined as any)).toThrow(
    "Invalid input: Text must be a string."
  );
  expect(() => processJustification(12345 as any)).toThrow(
    "Invalid input: Text must be a string."
  );
});
